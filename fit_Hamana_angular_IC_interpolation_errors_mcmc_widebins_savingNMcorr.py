...
#(61.064104495188595, 7.342517362169105e-07)
files_IC=glob.glob('/vol/fohlen11/fohlen11_1/bhernandez/data/corr/udropouts/m23t24.5/new/Wtheta_W*zrgu_weight_RR_udropouts_m23t24.5')
data_full=data
sigma=[]
DD=[]
RR=[]
while dif>0.001:
	...
	w=data_tmp[:,1]
	a=[idx for idx in range(len(w)) if w[idx] >0.009]
	...
	if np.isfinite(r0):
	    ...
	    DD.append(np.nanmean(data_IC[:-1,3]))
	    RR.append(np.nanmean(data_IC[:-1,-1]))

	
    IC_old=IC
    IC=1/np.sum(RR)*np.sum(np.array(sigma)*np.array(DD))
...
Nfile=open("N_23.0t24.5_2.dat", "a")
Mfile=open("Mhalo_23.0t24.5_2.dat", "a")
corrfile=open("correlations_23.0t24.5_2.dat", "a")
...
N_tmp=np.loadtxt('N_23.0t24.5_2.dat')
M_tmp=np.loadtxt('Mhalo_23.0t24.5_2.dat')
...
av_tmp=np.loadtxt('correlations_23.0t24.5_2.dat')
size=len(av_tmp)
sel=av_tmp[size/3.:]
size_sel=len(sel)
...
percentile=size_sel*0.20#0.16#size_sel/4
av_errm=sel[percentile]
av_errp=sel[size_sel*(0.95)]
#-sel[size_sel/2]
...
plt.title('23.0<r<24.5 udropouts')
...
plt.savefig('final_fit_230t245_2.eps', format='eps')
plt.show()
